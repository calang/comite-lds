...
- The site is published via GitHub Pages (Jekyll, `_config.yml` at `docs/_config.yml` — `docs/` is the Jekyll source, per `source: ./docs` in the deploy workflow, so build output paths mirror `docs/`'s contents directly with no `docs/` prefix, e.g. `../docs/Comité/agenda.md` publishes at `/comite/agenda.html`). GitHub's `github-pages` gem bundles `jekyll-optional-front-matter` and `jekyll-titles-from-headings`, so **any** `.md` anywhere under `docs/` gets converted to HTML and titled from its first `#` heading, front matter or not — front matter is only needed to set `layout: default` (minima's styling) and a specific `title:`. Two real controls, not front matter, decide what actually ships:
  ...
  - **Internal links between `.md` documents**: write them as a relative path to the target `.md` file (plus `#ancla` if linking to a heading), e.g. from `../docs/Comité/agenda.md` to `../docs/Comunidad/comisiones.md#3-comisión-de-tecnología` is `../comunidad/comisiones.md#3-comisión-de-tecnología`. Never link with `/docs/...` or an absolute path. The `github-pages` gem also bundles `jekyll-relative-links` (always on, not configurable) which rewrites these `.md` links to the rendered `.html` at build time — no script or config needed. Anchors must match the slug `kramdown` generates from the heading (lowercase, accents/spaces to hyphens — see the manual TOC in `Comisiones.md` for examples). This only applies to `.md` files under `docs/` (what Jekyll actually builds); a link from outside `docs/` (`README.md`, `specs/`) into `docs/` is not rewritten.
