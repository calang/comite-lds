...
- The site is published via GitHub Pages (Jekyll, `_config.yml` at repo root). GitHub's `github-pages` gem bundles `jekyll-optional-front-matter` and `jekyll-titles-from-headings`, so **any** `.md` anywhere in the repo gets converted to HTML and titled from its first `#` heading, front matter or not — front matter is only needed to set `layout: default` (minima's styling) and a specific `title:`. Two real controls, not front matter, decide what actually ships:
  - `exclude:` in `_config.yml` — anything not meant to be a public page (specs, scaffold dirs, README.md, etc.) must be listed there, or Jekyll will build and publish it.
  - `header_pages: []` in `_config.yml` — deliberately empty, so minima shows no top nav at all; without it, minima lists every titled page Jekyll finds, including anything not yet excluded.
  ...
  - The deploy workflow (`.github/workflows/pages.yml`) only triggers on push to `main` touching `docs/**`, `index.html`, or `_config.yml` — changes to *other* files that affect the site (e.g. a new `_config.yml` key, or excluding a new directory) won't redeploy unless they also touch one of those paths.
  - Run `make site` after adding or renaming documents to regenerate `index.html`.
