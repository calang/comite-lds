...
- The site is published via GitHub Pages (Jekyll, `_config.yml` at repo root). Any new `.md` file added under `docs/comite/` or `docs/comunidad/` needs minimal front matter (`---\nlayout: default\ntitle: ...\n---`) at the top: without it, Jekyll leaves the file as-is (`.md`, unconverted), but `index_builder.py` unconditionally links every indexed `.md` entry to its `.html` counterpart — so a missing front matter produces a dead link (404) on the published site, even though the file is still committed and viewable in the repo itself. Run `make site` after adding or renaming documents to regenerate `index.html`.
